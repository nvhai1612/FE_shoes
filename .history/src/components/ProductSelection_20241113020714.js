...
function ProductSelectionPage() {
  ...
  const [currentProduct, setCurrentProduct] = useState(null); // State to store the selected product details
  const [quantity, setQuantity] = useState(1); // State to handle quantity change in the modal
  ...
  const handleSelectChange = (product) => {
    ...
    setShowModal(true); // Show the modal when a product is selected
  };
