...
function MaterialList() {
  ...
  const [newMaterialStatus, setNewMaterialStatus] = useState("Đang có"); // Thêm state trạng thái
  ...
  const handleSaveNewMaterial = () => {
    if (newMaterialName.trim()) {
      const newMaterial = {
        ...
        status: newMaterialStatus, // Lưu trạng thái mới
      };
      setMaterials([...materials, newMaterial]);
      setNewMaterialName("");
      setNewMaterialStatus("Đang có"); // Reset trạng thái khi đóng modal
      ...
      <Modal show={showAddModal} onHide={() => setShowAddModal(false)}>
        ...
        <Modal.Body>
          <Form>
            ...
            {/* Trạng thái chất liệu */}
            <Form.Group controlId="newMaterialStatus" className="mt-3">
              <Form.Label>Trạng thái</Form.Label>
              <div style={{ display: "flex", gap: "10px" }}>
                <Form.Check
                  type="radio"
                  label="Đang có"
                  name="status"
                  checked={newMaterialStatus === "Đang có"}
                  onChange={() => setNewMaterialStatus("Đang có")}
                />
                <Form.Check
                  type="radio"
                  label="Ngừng cung cấp"
                  name="status"
                  checked={newMaterialStatus === "Ngừng cung cấp"}
                  onChange={() => setNewMaterialStatus("Ngừng cung cấp")}
                />
              </div>
            </Form.Group>
          </Form>
